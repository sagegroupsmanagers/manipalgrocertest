<?php
	...
	$query_category_id = mysqli_query($link,"select category_id from `oc_category_description` where `name` = '$sub_name'");
	
	if (!$query_category_id) {
    echo 'Could not run query: ';
    exit;
	}
	$row = mysqli_fetch_row($query_category_id);
	$cat_ID = $row[0];
	
	$query_product_id = mysqli_query($link,"SELECT p.product_id, p.price as mrp, p.image, pd.name , ps.price as sp,od.name as option_type, po.product_option_id from oc_product as p LEFT JOIN oc_product_to_category as pc on pc.product_id = p.product_id LEFT JOIN oc_product_description AS pd on pd.product_id = p.product_id LEFT JOIN oc_product_special AS ps on ps.product_id = p.product_id LEFT JOIN oc_product_option as po on po.product_id = p.product_id LEFT JOIN oc_option_description as od on od.option_id = po.option_id where pc.category_id='$cat_ID'");
